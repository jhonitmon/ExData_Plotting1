library("dplyr")
library("lubridate")
hpc_df <- read.csv("tidydata.csv")
hpc_df$Date <- ymd(hpc_df$Date)
hpc_df$Time <- ymd_hms(hpc_df$Time)
date(hpc_df$Time) <- hpc_df$Date

png("plot4.png")
par(mfrow = c(2,2))
with(hpc_df,plot(Time,Global_active_power,type = "l", xlab ="", ylab = "Global Active Power"))
with(hpc_df,plot(Time,Voltage,type = "l", xlab ="datetime", ylab = "Voltage"))
plot(hpc_df$Time,hpc_df$Sub_metering_1,type = "l",xlab = "", ylab = "Energy sub metering")
lines(hpc_df$Time,hpc_df$Sub_metering_2,col = "red")
lines(hpc_df$Time,hpc_df$Sub_metering_3,col = "blue")
legend("topright", legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), col = c("black","red","blue"), lty = 1, bty = "n")
with(hpc_df,plot(Time,Global_reactive_power,type = "l", xlab ="datetime", ylab = "Global_reactive_power"))
dev.off()